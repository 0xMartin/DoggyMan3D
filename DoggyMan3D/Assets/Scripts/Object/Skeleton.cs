...
public class Skeleton : AICharacterBase
{
    ...
    {
        // walk
        _animator.SetFloat("speedv", _gameEntity.IsMoving() || _gameEntity.IsSprinting() ? 2.0f : 0.0f);

        // utok
        _animator.SetBool("Attack", _gameEntity.IsAttacking());
        AnimatorStateInfo info = _animator.GetCurrentAnimatorStateInfo(0);
        _gameEntity.IsEnabledMoving = !info.IsName("Attack1h1");
    ...
    private void DeathAnimationStart()
    {
        ...
        StartCoroutine(DeathAnimationAsync());
    }

    private IEnumerator DeathAnimationAsync()
    {
        for (int i = 0; i < 60; ++i)
        {
            SkeletonBody.transform.position = new Vector3(
                SkeletonBody.transform.position.x, 
                SkeletonBody.transform.position.y - 0.025f, 
                SkeletonBody.transform.position.z);
